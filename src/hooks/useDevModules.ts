  ...
  const refreshModules = async () => {
    ...
    try {
      ...
      const { data: dbModules, error } = await supabase
        ...
        .select('*')
        .eq('is_active', true)
        .neq('development_stage->stage', 'production'); // Exclude production modules
      ...
      console.log(`📋 Development modules found: ${dbModules?.length || 0}`);
      ...
      const modules: LoadedModule[] = (dbModules || [])
        .filter(dbModule => {
          // Filter out modules that are clearly system/production modules
          const developmentStage = dbModule.development_stage || {};
          const stage = typeof developmentStage === 'object' && developmentStage.stage 
