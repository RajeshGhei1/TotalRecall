...
CREATE TABLE IF NOT EXISTS public.ai_decision_instances (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID REFERENCES public.tenants(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.ai_decision_workflows (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID REFERENCES public.tenants(id) ON DELETE CASCADE,
  name TEXT NOT NULL,
  description TEXT,
...
CREATE TABLE IF NOT EXISTS public.ai_decision_analytics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID REFERENCES public.tenants(id) ON DELETE CASCADE,
  rule_id UUID REFERENCES public.ai_decision_rules(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_ai_decision_rules_tenant ON public.ai_decision_rules(tenant_id);
CREATE INDEX IF NOT EXISTS idx_ai_decision_rules_module ON public.ai_decision_rules(module_context);
CREATE INDEX IF NOT EXISTS idx_ai_decision_rules_active ON public.ai_decision_rules(is_active);
CREATE INDEX IF NOT EXISTS idx_ai_decision_rules_type ON public.ai_decision_rules(rule_type);

CREATE INDEX IF NOT EXISTS idx_ai_decision_instances_tenant ON public.ai_decision_instances(tenant_id);
CREATE INDEX IF NOT EXISTS idx_ai_decision_instances_rule ON public.ai_decision_instances(rule_id);
CREATE INDEX IF NOT EXISTS idx_ai_decision_instances_result ON public.ai_decision_instances(decision_result);
CREATE INDEX IF NOT EXISTS idx_ai_decision_instances_created ON public.ai_decision_instances(created_at);

CREATE INDEX IF NOT EXISTS idx_ai_decision_approvals_instance ON public.ai_decision_approvals(decision_instance_id);
CREATE INDEX IF NOT EXISTS idx_ai_decision_approvals_status ON public.ai_decision_approvals(approval_status);
CREATE INDEX IF NOT EXISTS idx_ai_decision_approvals_approver ON public.ai_decision_approvals(approver_id);

CREATE INDEX IF NOT EXISTS idx_ai_decision_workflows_tenant ON public.ai_decision_workflows(tenant_id);
CREATE INDEX IF NOT EXISTS idx_ai_decision_workflows_active ON public.ai_decision_workflows(is_active);

CREATE INDEX IF NOT EXISTS idx_ai_decision_analytics_rule_date ON public.ai_decision_analytics(rule_id, date);
CREATE INDEX IF NOT EXISTS idx_ai_decision_analytics_tenant_date ON public.ai_decision_analytics(tenant_id, date);
...
DROP POLICY IF EXISTS "Tenant admins can manage their tenant's decision rules" ON public.ai_decision_rules;
...
DROP POLICY IF EXISTS "Users can view their tenant's decision rules" ON public.ai_decision_rules;
...
DROP POLICY IF EXISTS "Tenant admins can manage their tenant's decision instances" ON public.ai_decision_instances;
...
DROP POLICY IF EXISTS "Users can view their tenant's decision instances" ON public.ai_decision_instances;
...
DROP POLICY IF EXISTS "Users can manage their own approvals" ON public.ai_decision_approvals;
CREATE POLICY "Users can manage their own approvals"
  ON public.ai_decision_approvals
  FOR ALL
  USING (approver_id = auth.uid());

DROP POLICY IF EXISTS "Tenant admins can view all approvals in their tenant" ON public.ai_decision_approvals;
...
DROP POLICY IF EXISTS "Tenant admins can manage their tenant's workflows" ON public.ai_decision_workflows;
...
DROP POLICY IF EXISTS "Users can view their tenant's workflows" ON public.ai_decision_workflows;
...
DROP POLICY IF EXISTS "Tenant admins can view their tenant's analytics" ON public.ai_decision_analytics;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE pubname = 'supabase_realtime'
      ...
      AND tablename = 'ai_decision_instances'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE pubname = 'supabase_realtime'
      AND schemaname = 'public'
      AND tablename = 'ai_decision_approvals'
  ) THEN
